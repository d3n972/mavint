...
func GTFSUpdaterTask() *Schedule {
	return &Schedule{
		Interval: 24 * time.Hour,
		Handler: func(ctx AppContext) {
			client := &http.Client{}
			req, _ := http.NewRequest("GET", "https://www.mavcsoport.hu/gtfs/gtfsMavMenetrend.zip", nil)
			req.Header.Add("Authorization", "Basic "+basicAuth("mark.adonyi@gmail.com", "j9i1pwCTNcEx9eRd"))
			...
			if zipBytes, e := io.ReadAll(resp.Body); e == nil {
				...
				unzip("/tmp/mavgtfs.zip", "/var/lib/gtfs")
				if _, err := exists("/var/lib/gtfs"); err == fs.ErrNotExist {
